...
CTR Prediction System (Intent-Aware)

Taobao Dataset (User / Ad / Behavior Logs)
│
├── Data Processing Pipeline
│   ├── Behavior Aggregation
│   │   ├── total_pv, total_cart, total_buy
│   │   ├── buy_rate, cart_rate
│   │   └── action_group (sparse / medium / dense)
│   │
│   ├── Feature Engineering
│   │   ├── pv_to_cart_rate
│   │   ├── cart_to_buy_rate
...
│   │
│   └── Intent Embedding
│       ├── behavior → text template
│       ├── Sentence Transformer
│       └── PCA
│
├── Modeling
│   ├── Model A (Baseline)
│   │   └── DeepFM (structured features only)
│   │
│   ├── Model B (Enhanced)
│   │   └── DeepFM + intent embedding + ad embedding
│   │
│   └── Model C (Upper Bound - optional)
│       └── BST (sequential model, to be added)
│
├── Training Pipeline
│   ├── Dataset (CTRDataset)
│   ├── DataLoader
│   ├── Training Loop (Adam + Scheduler)
│   └── Prediction (CTR score)
│
├── Evaluation
│   ├── AUC
│   ├── Log Loss
│   ├── ECE
│   └── Group-wise Evaluation (sparse / medium / dense)
│
└── Output
    ├── Prediction scores
    ├── Performance metrics
